...
@torch.no_grad()
def extract_features1(model, data_loader):
    metric_logger = utils.MetricLogger(delimiter="  ")
    features = []
    labels = []
    for samples, label in metric_logger.log_every(data_loader, 10):
        samples = samples.cuda(non_blocking=True)
        feats = model(samples).detach().cpu().numpy()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--data_path', default='/tmp/zhongz2/images', type=str)
    parser.add_argument('--dataset', default='mnist', type=str, choices=['mnist', 'roxford5k', 'rparis6k'])
    parser.add_argument('--multiscale', default=False, type=utils.bool_flag)
    parser.add_argument('--imsize', default=224, type=int, help='Image size')
    parser.add_argument('--pretrained_weights', default='./saving_dir_mnist/checkpoint.pth', type=str, help="Path to pretrained weights to evaluate.")
    ...
    parser.add_argument("--savefilename", type=str, default="")
    ...
    dataset_train = ImageFolder(root=os.path.join(args.data_path, 'train'), transform=transform)
    dataset_query = ImageFolder(root=os.path.join(args.data_path, 'val'), transform=transform)
    ...
    query_features, query_labels = extract_features1(model, data_loader_query)
    with open(args.savefilename, 'wb') as fp:
        pickle.dump({'feats': query_features, 'labels': query_labels}, fp)

    # train_features, train_labels = extract_features1(model, data_loader_train)
    # with open('train_feats.pkl', 'wb') as fp:
    #     pickle.dump({'feats': train_features, 'labels': train_labels}, fp)
